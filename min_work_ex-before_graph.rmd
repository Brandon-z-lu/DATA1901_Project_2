...
get_l_suburb <- function(location, suburb_lat, suburb_lon) {
  df_suburb <- house_scraping(location)
  ...
  file_name <- paste0("l_", gsub("/", "_", location), "_houseprice.csv")
  print(file_name)
  ...
  print("Finish csv export")
}
get_l_suburb("2122/eastwood/", -33.7899, 151.0821)
get_l_suburb("2144/auburn/", -33.8490, 151.0329)
...
<!-- get_l_suburb <- function(location, suburb_lat, suburb_lon) { -->
<!--   df_suburb <- house_scraping(location) -->
<!--   l_suburb <- df_suburb %>% mutate(address = as.character(address)) %>% geocode(address, method = 'arcgis', lat=latitude, long=longitude) -->
...
<!--   } -->
<!--   file_name <- paste0("l_", gsub("/", "_", location), "_houseprice.csv") -->
<!--   file_path <- file.path("csv_cache", file_name) -->
<!--   write.csv(l_suburb_dist, file_path, row.names = FALSE) -->
